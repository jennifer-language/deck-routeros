...
if (len($dsls) == 0) { io.printf("no PPPoE clients\n"); }
for (def d in $dsls) {
    if ($d.running) { io.printf("pppoe %s: connected as %s over %s\n", $d.name, $d.user, $d.interfaceName); }
    else { io.printf("pppoe %s: down\n", $d.name); }
}
...
for (def u in $vusers) { io.printf("vpn user %s (service %s, profile %s)\n", $u.name, $u.service, $u.profile); }
...
for (def s in $srvs) { io.printf("pppoe server \"%s\" on %s\n", $s.serviceName, $s.interfaceName); }
