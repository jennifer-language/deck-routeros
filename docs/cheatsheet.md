...
[index.md](index.md); runnable per-topic examples are in
[../examples/](../examples/).
...
`update(c, path, id, attrs)` (synonym of `set`), `remove(c, path, id)` (RouterOS's delete),
...
`interfaceByName(c, name)`, `enableInterface(c, name)` / `disableInterface(c, name)`,
...
`removeFromInterfaceList(c, listName, interfaceName)` /
`removeInterfaceList(c, name)`. Firewall rules match a
...
`enableLte(c, name)` / `disableLte(c, name)`.
...
serving another bond. `bonds(c)` → `list of Bond`, `bondByName(c, name)`,
`setBondSlaves(c, name, slaves)`, `removeBond(c, name)` /
`enableBond(c, name)` / `disableBond(c, name)`; constants `BOND_LACP` / `BOND_ACTIVE_BACKUP`.
...
as an int), `vlanByName(c, name)`, `removeVlan(c, name)` /
`enableVlan(c, name)` / `disableVlan(c, name)`.
...
`firewallRule(chain, action)` and the value-semantic refiners - each
takes the rule and returns a refined copy: `withProtocol(rule, protocol)`,
`withSrcAddress(rule, address)`, `withDstAddress(rule, address)`,
`withSrcPort(rule, spec)`, `withDstPort(rule, spec)`,
`withInInterface(rule, name)`, `withOutInterface(rule, name)`,
`withComment(rule, comment)`, `withDisabled(rule, disabled)`; then
`addFirewallRule(c, rule)` → id. Read back with `firewallRules(c)`;
manage by the rule's comment with `removeFirewallRuleByComment(c, comment)`,
`enableFirewallRuleByComment(c, comment)`,
`disableFirewallRuleByComment(c, comment)`, or by id with
`removeFirewallRule(c, id)`.
...
`withInInterfaceList(rule, listName)` / `withOutInterfaceList(rule, listName)`
(see Interface lists).
...
with `removeNatRuleByComment(c, comment)` /
`enableNatRuleByComment(c, comment)` /
`disableNatRuleByComment(c, comment)` (temporarily close a forward
without losing it), or by id with `removeNatRule(c, id)`. Order matters
in `dstnat` too: `moveNatRule(c, id, beforeId)` /
`moveNatRuleByComment(c, comment, beforeComment)`.
...
refiners `withSrcAddressList(rule, listName)` /
`withDstAddressList(rule, listName)`.
...
by their comment convention. Mangle owns the extra chain constant
`CHAIN_POSTROUTING` (marking on the way out), and its table is ordered
like the others: `moveMangleRule(c, id, beforeId)` /
`moveMangleRuleByComment(c, comment, beforeComment)`.
...
`removeDhcpClient(c, interfaceName)` /
`enableDhcpClient(c, interfaceName)` /
`disableDhcpClient(c, interfaceName)` manage by interface. Together with NAT this is the
...
`removePppoeClient(c, name)` / `enablePppoeClient(c, name)` /
`disablePppoeClient(c, name)`. The password is write-only - it never appears in a struct.
...
`vpnUsers(c)`, `removeVpnUser(c, name)`, and `enableVpnUser(c, name)` /
`disableVpnUser(c, name)` to park a login without losing it;
`pppActive(c)` → `list of PppSession` for who is connected,
`kickPppUser(c, name)`). `pppProfiles(c)` → `list of PppProfile` lists
the profiles those sessions get their addresses and DNS from. Turn a
server on, then add users: `enableL2tpServer(c, ipsecSecret)`
(L2TP/IPsec - native on every OS; opens IKE/NAT-T/ESP + udp 1701),
`enableSstpServer(c, certificate, port)` (TLS on 443, crosses
firewalls), `enableOvpnServer(c, certificate, port)` (OpenVPN) - each
with a matching `disableL2tpServer(c)` / `disableSstpServer(c)` /
`disableOvpnServer(c)` that also removes its firewall openings, and
`l2tpServerStatus(c)` / `sstpServerStatus(c)` / `ovpnServerStatus(c)` to
read back whether it is running. TLS servers need a certificate
(certificates topic). Dial OUT with the same shape on all three -
`addL2tpClient(c, name, serverAddress, user, password)` /
`addSstpClient(c, name, serverAddress, user, password)` /
`addOvpnClient(c, name, serverAddress, user, password)` - listed by
`l2tpClients(c)` / `sstpClients(c)` / `ovpnClients(c)` and dropped by
`removeL2tpClient(c, name)` / `removeSstpClient(c, name)` /
`removeOvpnClient(c, name)`. `setupIkev2Server(c, name, certificate,
poolRange, dns)` covers native IKEv2 road-warriors (ipsec topic).
...
Operations: `hotspotServers(c)` → `list of HotspotServer` (what is
serving a portal), `hotspotActive(c)` → `list of HotspotSession` (usage
...
`removeRoute(c, dstAddress)` / `enableRoute(c, dstAddress)` /
`disableRoute(c, dstAddress)` (match the first *static* route to that
destination; dynamic routes are never touched). `DEFAULT_ROUTE` is the
`"0.0.0.0/0"` constant to match on. The gateway may be an IP address or an interface name - if it
...
`removeSimpleQueue(c, name)` / `enableSimpleQueue(c, name)` /
`disableSimpleQueue(c, name)` manage it by name.
...
(removes the whole subtree, children first), `enableTreeQueue(c, name)` /
...
`enableWifi(c, interfaceName)` / `disableWifi(c, interfaceName)`,
...
remote server with NAT-friendly keepalive; `addWireguard(c, name,
listenPort)` → id makes the bare interface when you would rather wire
the address and firewall yourself. `wireguardInterfaces(c)` →
`list of WireguardInterface` / `wireguardPeers(c)` report state
(`lastHandshake` is the liveness signal),
`removeWireguardPeerByComment(c, comment)` and
`removeWireguard(c, name)` (also cleans up the address and firewall
rule) tear down, plus `enableWireguard(c, name)` /
`disableWireguard(c, name)`.
...
`eoipTunnelByName(c, name)`, `removeEoipTunnel(c, name)` /
`enableEoipTunnel(c, name)` / `disableEoipTunnel(c, name)`.
...
`greTunnels(c)` → `list of GreTunnel`, `greTunnelByName(c, name)`,
`removeGreTunnel(c, name)` / `enableGreTunnel(c, name)` /
`disableGreTunnel(c, name)`.
...
`established`), `ipsecPolicies(c)` → `list of IpsecPolicy` (per-policy
`active` flag), `ipsecPeers(c)` → `list of IpsecPeer`. For native
road-warrior clients (iOS/macOS/Windows
...
address, plus `enableVrrp(c, name)` / `disableVrrp(c, name)`.
`addVrrp(c, name, interfaceName, vrid, priority)` → id creates the bare
instance when you want to place the shared address yourself.
...
`activeUsers(c)` → `list of UserSession`;
...
`setUserPassword(c, name, password)`, `setUserGroup(c, name, group)`,
`restrictUser(c, name, addresses)` (limit login sources to IPs/CIDRs,
`""` lifts it), `removeUser(c, name)`, `enableUser(c, name)` /
`disableUser(c, name)`. The `Client` knows which account it is
...
`enableService(c, name)` / `disableService(c, name)` (mind the docblock: don't
...
issuer, `hasPrivateKey`, expiry), `certificateByName(c, name)`,
...
`readFileText(c, name)` / `writeFileText(c, name, contents)` (small text files,
...
`startContainer(c, name)` / `stopContainer(c, name)` /
`removeContainer(c, name)`. Config rides on named lists:
`addContainerEnv(c, listName, key, value)` and
`addContainerMount(c, listName, src, dst)` → id, attached with
`setContainerEnvLists(c, name, listNames)` /
`setContainerMountLists(c, name, listNames)` *before* the first start;
`containerEnvs(c)` / `containerMounts(c)` list them, and
`removeContainerEnvList(c, listName)` /
`removeContainerMountList(c, listName)` → count clear one for an
idempotent re-run. `Container.name` is the
...
`SnmpSettings`, `snmpCommunities(c)` → `list of SnmpCommunity`,
`disableSnmp(c)`.
...
`disableWatch(c, host)` / `enableWatch(c, host)` - all keyed by host.
...
int `runCount`), and `removeScheduledTask(c, name)` /
`enableScheduledTask(c, name)` / `disableScheduledTask(c, name)` manage
by name. The script runs with
...
`import "@jennifer/routeros/"` (the trailing slash resolves to that entry
module). The topic files are not standalone modules.
